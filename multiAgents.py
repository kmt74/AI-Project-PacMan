...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState: GameState, action):
        ...
        for ghostState, scaredTime in zip(newGhostStates, newScaredTimes):
            ...
            if scaredTime == 0 and ghostDist > 0:
                if ghostDist <= 1:
                    score -= 1000
                else:
                    score -= 1.0 / ghostDist
            elif scaredTime > 0 and ghostDist > 0:
                score += 1.0 / ghostDist
        ...
        def minimax(state, agentIndex, depth):
            ...
            if agentIndex == 0:
                ...
                for action in state.getLegalActions(agentIndex):
                    ...
                    bestVal = max(bestVal, minimax(successor, 1, depth))
                return bestVal
            else:
                ...
                for action in state.getLegalActions(agentIndex):
                    successor = state.generateSuccessor(agentIndex, action)
                    bestVal = min(bestVal, minimax(successor, nextAgent, nextDepth))
                return bestVal
...
def betterEvaluationFunction(currentGameState: GameState):
    ...
    DESCRIPTION:
    Improved logic to fix the "lazy/standing still" Pacman issue.
    1. Food Incentive: Increased the weight of minFoodDist to 10.0 to create a steep,
       irresistible gradient towards food, overpowering minor ghost probabilities.
    2. Anti-Thrashing: Heavily penalized the STOP action to force movement.
    3. Buffer Zones: Added a secondary warning zone (dist <= 2) to avoid getting cornered.
    """
